...
namespace wr_anit_cushman_one.Controllers
{
    ...
    public class Terrenos1Controller : Controller
    {
...

        {
            ...
            tsg001_terreno tsg001_terreno = null;
            tsg023_ni_precio tsg023_te_precio = null;
            tsg026_te_dt_gral tsg026_te_dt_gral = null;
            tsg027_te_servicio tsg027_te_servicio = null;
            tsg028_te_contacto propietario = null;
            tsg028_te_contacto administrador = null;
            tsg028_te_contacto corredor = null;
            ...
            else
            {
                ...
                int cd_precio = db.tsg023_ni_precio.Where(x => x.cd_terreno == id).Max(x => x.cd_ni_precio);
                tsg023_te_precio = db.tsg023_ni_precio.Find(cd_precio);
                int cd_propietario = db.tsg028_te_contacto.Where(x => x.cd_terreno == id && x.st_contacto == 1).Max(x => x.cd_contacto);
                propietario = db.tsg028_te_contacto.Find(cd_propietario);
                int cd_administrador = db.tsg028_te_contacto.Where(x => x.cd_terreno == id && x.st_contacto == 2).Max(x => x.cd_contacto);
                administrador = db.tsg028_te_contacto.Find(cd_administrador);
                int cd_corredor = db.tsg028_te_contacto.Where(x => x.cd_terreno == id && x.st_contacto == 3).Max(x => x.cd_contacto);
                ...
                if (t != null)
                {
                    try
                    {
                        ...
                        if (t.nb_posicion != null || t.nb_poligono != null)
                        {
                            ...
                            iTextSharp.text.Image mapa1 = iTextSharp.text.Image.GetInstance("https://maps.googleapis.com/maps/api/staticmap?center=" + t.nb_posicion + "&zoom=13&size=640x500&maptype=hybrid&path=fillcolor:red|" + t.nb_poligono.TrimEnd('|') + "&key=AIzaSyDhfzatgC_64higR2LlLGkWldmTBudiRWY");
            ...
            else// Ingles
            {
                ...
                if (t != null)
                {
                    try
                    {
                        ...
                        {
                            //table.AddCell("Informaciòn de la propiedad");                    
                            iTextSharp.text.Image mapa1 = iTextSharp.text.Image.GetInstance("https://maps.googleapis.com/maps/api/staticmap?center=" + t.nb_posicion + "&zoom=17&size=640x500&maptype=satellite&path=fillcolor:red|" + t.nb_poligono.TrimEnd('|') + "&key=AIzaSyDhfzatgC_64higR2LlLGkWldmTBudiRWY");
