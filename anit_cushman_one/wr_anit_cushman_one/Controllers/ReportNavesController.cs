    ...
    public class ReportNavesController : Controller
    {
        ...
        public ActionResult ReportePP(int cd_estado, int cd_municipio, int cd_colonia)
        {
            ...
            try
            {
                ...
                // PPTReport.RunReportPPt.setRporte(ruta);
                descargaImagenes(cd_estado, cd_municipio, cd_colonia, ruta);
                PPTReport.clases.reporte_pp.rep_powerpoint(cd_estado, cd_municipio, cd_colonia, ruta);

                uploadfileppt(Server.MapPath("~/repositorio/" + "reporte.pptx"));
                util.EliminaTodosArchivos(ruta);
        ...
        private void descargaImagenes(int cd_estado, int cd_municipio, int cd_colonia, string ruta)
        {
            try
            {
                var naves = from n in db.tsg002_nave_industrial
                ...
                if (cd_municipio != 0)
                {
                    naves = naves.Where(t => t.cd_estado == cd_estado && t.cd_municipio == cd_municipio);

                }

                if (cd_colonia != 0)
                {
                    naves = naves.Where(t => t.cd_estado == cd_estado && t.cd_municipio == cd_municipio && t.cd_colonia == cd_colonia);
                }
                utils util = new utils();
                int cont=0;
                foreach (var nave in naves)
                {
                    try
                    {
                        var imagenes = db.tsg045_imagenes_naves.Where(t => t.cd_nave == nave.cd_nave && t.cd_reporte == 1);
                        if (imagenes.Count() > 0)
                        {
                            string urlImage = descargaBlob(nave.cd_nave, imagenes.First().nb_archivo);
                            string rutaArchivo = util.GeneraImagen(urlImage, "N_" + nave.cd_nave, ruta);
                            cont++;
                        }
                    }
                    catch (Exception ex)
        ...
        private void GeneraPDF(int cd_estado, int cd_municipio, int cd_colonia, string cd_titulo, string cd_idioma)
            ...
            var edo = "";
                ...
                //Inicia for para pintar los títulos
                foreach (var titulos in terrenos)
                {
                    ...
                    table.AddCell(cell1);
                    coordenadas = coordenadas + (titulos.nb_posicion != null ? titulos.nb_posicion.TrimEnd('|') : "") + "|";
                    posiciones = posiciones + "markers=color:red%7Clabel:" + ObteneLetra(j) + "%7C" + (titulos.nb_posicion != null ? titulos.nb_posicion.TrimEnd('|') : "") + "&";
                ...
                //Fin for para pintar los títulos
                ...
                foreach (var terreno in terrenos)
                {
                    ...
                    string strUrlPoligono = "https://maps.googleapis.com/maps/api/staticmap?center=" + (terreno.nb_posicion!=null? terreno.nb_posicion:"") + "&zoom=15&size=840x640&maptype=hybrid&path=fillcolor:red|" + (terreno.nb_poligono!=null? terreno.nb_poligono.TrimEnd('|'):"") + "&key=AIzaSyDhfzatgC_64higR2LlLGkWldmTBudiRWY";
                ...
                if (precio.im_venta > 0) {
                    ...
                    nested2.AddCell(new Phrase(" - Altura libre / Clear Height:" + String.Format("{0:#,##0.00}", datos_gral.nu_altura) + " M / " + String.Format("{0:#,##0.00}", Convert.ToDouble(datos_gral.nu_altura) * 3.28084) + " Ft\n", new Font(Vn_Helvetica, 9, Font.NORMAL, BaseColor.BLACK))); 
